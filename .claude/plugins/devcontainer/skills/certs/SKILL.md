...
Section 5.5's own block, reproduced here as a table, one row per file:

| File | Path | Mode | Leaves the laptop | Lifetime |
|---|---|---|---|---|
| CA private key | `~/.docker/certs/<instance>/ca/ca-key.pem` | `0600` | Never | `CERT_CA_DAYS` |
| CA public certificate | `~/.docker/certs/<instance>/ca/ca.pem` | `0644` | Published as `String` to `/devcontainer/<instance>/tls/ca.pem` (Section 5.3) | `CERT_CA_DAYS` |
| Client certificate | `~/.docker/certs/<instance>/cert.pem` | `0644` | Never -- Section 5.3 names no parameter path for it, so it is presented only over the TLS handshake itself, never persisted to the store | `CERT_CLIENT_DAYS` |
| Client private key | `~/.docker/certs/<instance>/key.pem` | `0600` | Never | `CERT_CLIENT_DAYS` |
