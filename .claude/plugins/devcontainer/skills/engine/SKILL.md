...
`certs` and `instances` modules (spec Section 4.5); both now exist. `certs`
(E6-F1-S1-T1) now exists for certificate generation, and its
inspection/expiry-status half (`certs.classify`/`certs.status_rows`,
E6-F1-S1-T2) now exists too, for the `client` and `ca` roles it persists
locally; the SSM port forward manager (`transport`, E6-F2-S1-T1) now exists
too, for SSM agent status, per-instance local port allocation, the
`aws ssm start-session` argument vector and its readiness detection.
`instances` (E8-F1-S1-T1) now exists too, and owns discovery, naming and the
Section 9 addressing table (spec Section 4.5): every `## Checks` row and
`## Procedure` step below that needed remote instance resolution now calls
`instances.resolve` directly rather than naming `instances` as a future
owner. `instances` does not probe EC2 running state, so the one instance
fact still missing -- whether the resolved instance actually exists and is
running -- remains a host fact no module in this repository probes yet,
alongside the two above; this document's introduction names `hostprobe` as
the module Section 4.5's host-fact responsibility extends to once that probe
is implemented, the same way it already does for disk headroom and
`HOST_PROXY`, rather than a function this repository does not yet contain.
Naming the owner is this row's job; implementing the probe is a later work
unit's.
...
| instance exists, is running, SSM agent online | Depends on: `SSO session valid`. Prevents the port forward and the docker version handshake, since neither can reach an instance whose SSM agent is not reporting online -- `instances` (E8-F1-S1-T1) now resolves which instance this check names (`instances.resolve`) but its spec Section 4.5 responsibility is discovery, naming and the Section 9 addressing table, not EC2 running state, so instance existence and running state remain a host fact no function in this repository probes yet, per this document's introduction; the SSM-agent-online half is owned by `transport.ensure_agent_online` (E6-F2-S1-T1), which already exists. | The resolved `remote_instance_id` does not resolve to a running instance, or the instance is running but its SSM agent is not online (`transport.ensure_agent_online` raising for any status other than `Online`, naming the reported ping status or that SSM has no record of the instance at all). | Stop, name the instance id and the agent state, and state that the port forward cannot be established until the agent reports online -- an operator action; this skill does not attempt to start an instance or its SSM agent itself. Do not attempt the forward: its failure would name the wrong cause. |
| port forward established and the local port answering | Depends on: `instance exists, is running, SSM agent online`. Prevents `make build INSTANCE=<name>`, and every command that needs the daemon reachable through the tunnel, from ever reaching the instance -- owned by `transport.start_forward` and `transport.wait_ready` (E6-F2-S1-T1). | The SSM port forward to the instance's `DOCKER_TLS_PORT` did not answer within `SSM_FORWARD_TIMEOUT` (the same deadline `/devcontainer:setup-remote` step 12 already waits on, spec Section 7.3). | This skill re-establishes the forward itself -- the second of the two actions `## What this skill fixes itself` names, since it is reversible and needs no operator credential beyond the already-verified SSO session. If the forward still does not answer after being re-established, stop, name the instance and the allocated local port (spec Section 9: "never a fixed number"), and state that the checks depending on this one (the tunnel handshake, remote disk headroom, unpushed work) were not run. |
| client and CA certificates present and unexpired | Depends on: none among the remote checks above -- reads only the local certificate files under `instances.certs_root()`'s `<name>/` directory (`$DOCKER_CONFIG/certs/<name>/`, or `~/.docker/certs/<name>/` when `DOCKER_CONFIG` is unset) once `<name>` is resolved (Section 4.1.1); needs neither the SSO session, the instance, nor the forward. Prevents `make build INSTANCE=<name>` presenting an expired or absent client certificate to a server that refuses it -- implemented by `certs.status_rows` and `certs.classify` (E6-F1-S1-T2), the identical function `make cert-status` calls, over the `client` and `ca` roles: the two certificates persisted under `instances.certs_root()`'s `<name>/` directory (`$DOCKER_CONFIG/certs/<name>/`, or `~/.docker/certs/<name>/` when `DOCKER_CONFIG` is unset), per `certs.py`'s own module docstring. The server certificate has no local file to inspect -- it is generated and published in one step with no persisted path (`certs.issue_server`, E6-F1-S1-T1) -- so this check, like `make cert-status`, does not cover it. | No certificate exists at the expected path under `instances.certs_root()`'s `<name>/` directory (`$DOCKER_CONFIG/certs/<name>/`, or `~/.docker/certs/<name>/` when `DOCKER_CONFIG` is unset), or the client or CA certificate has already expired. A certificate that has not yet expired but expires within `CERT_WARN_DAYS` is a `WARN`, not this failure; see `## Verdict`. | Issue or renew the missing or expired certificate with `certs.create_ca` or `certs.issue_client` (E6-F1-S1-T1). State the exact reissue action, wait for the operator to run it, then re-check the certificate's expiry before continuing -- an operator action. Never proceeds on the assumption that reissuing succeeded. |
...
1. Resolve which engine this run validates. Probe the active docker context
   ...
   resolved backend is remote, resolve the instance with `instances.resolve`
   (E8-F1-S1-T1), the single place spec Section 4.1.1's four-step order and
   its three edge cases are implemented; an `AmbiguousInstanceError` becomes
   the only question this skill ever asks. When the resolved backend is
   local, no instance resolution happens and only the six local checks below
   run.
...
10. Run `instance exists, is running, SSM agent online`: existence and
    running state remain a host fact no function in this repository probes
    yet, per this document's introduction; `instances` (E8-F1-S1-T1)
    resolves which instance this check names but its spec Section 4.5
    responsibility stops at discovery, naming and the addressing table.
    The SSM-agent-online half is confirmed by calling
    `transport.ensure_agent_online` (E6-F2-S1-T1), which already exists.
    Apply this row's remedy on failure, and do not run steps 11, 13, 14 and
    15 until it passes. Step 12, the certificate check, remains independent
    for the same reason step 9 states, so it still runs.
11. Run `port forward established and the local port answering` (owned by
    `transport.start_forward` and `transport.wait_ready`, E6-F2-S1-T1). On
    failure, apply this row's self-fix remedy: re-establish the forward itself, then
...
12. Run `client and CA certificates present and unexpired` against the local
    certificate files under `instances.certs_root()`'s `<name>/` directory (`$DOCKER_CONFIG/certs/<name>/`, or `~/.docker/certs/<name>/` when `DOCKER_CONFIG` is unset), via `certs.status_rows`
...
every still-missing probe named in `## Checks` exists: `hostprobe`'s
disk-headroom and `HOST_PROXY` probes, and the EC2 existence/running-state
probe this document's introduction names `hostprobe` as owning by
extension. `instances` (E8-F1-S1-T1) already exists and resolves the
instance name and the addressing block the transcript's `instance
i-0abc123` row displays; only whether that instance is actually running is
still unprobed. The inspection/expiry-status half of `certs`
(`certs.status_rows`/`certs.classify`, E6-F1-S1-T2) now exists too, and so
does the SSM port forward manager
(`transport.start_forward`/`transport.wait_ready`, E6-F2-S1-T1), so the
certificate row and the port forward row below are both live rather than
one of the remaining `NOT RUN` rows. Until each of the still-missing
probes exists, the row for the check it owns reports `NOT RUN` in place
of the value shown here, per
...
- Section 4.4: `GATE-APPLY`, `GATE-CUTOVER` and `GATE-DESTROY`, none of which
  this skill reaches.
- Section 4.5: `verify` and `hostprobe` decide a fact about a rendered file
  or this machine; `certs` (E6-F1-S1-T1) now owns certificate generation,
  and its inspection/expiry-status half (`certs.classify`/`certs.status_rows`,
  E6-F1-S1-T2) now owns the certificate-expiry facts for the `client` and
  `ca` roles it persists locally; `transport` (E6-F2-S1-T1) now owns the SSM
  port forward facts: SSM agent status, per-instance local port allocation,
  and forward readiness; and `instances` (E8-F1-S1-T1) now owns discovery,
  naming and the Section 9 addressing table. Whether the resolved instance
  actually exists and is running remains the one instance fact none of them
  decide yet; this document's introduction names `hostprobe` as the module
  Section 4.5 will extend to cover it. This skill decides none of them
  itself.
